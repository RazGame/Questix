 ...
 *
 * Сейчас настроен на блок «Вне времени — кино, сериалы и реклама»:
 * «Стыцамэн» и «Прасковья» туда попали по недоразумению — ни та, ни другая
 * ни к какому фильму не относятся (потому и остались без пометки, когда
 * я расставлял подсказки). Меняем на русскую киноклассику, которую зал
 * узнаёт с первых нот.
 *
...
const BLOCK = 'Вне времени — кино, сериалы и реклама';

// кого убрать (исполнитель + название, как в игре)
const REMOVE = [
  { a: 'Иван Дорн', t: 'Стыцамэн' },
  { a: 'UMA2RMAN', t: 'Прасковья' },
];

// кого поставить. note — пометка ведущему: угадывают название песни,
// а фильм нужен ему самому для контекста и разговора с залом.
const ADD = [
  {
    a: 'Sergey Shnurov', t: 'Мобильник', start: 20,
    note: 'фильм «Бумер» — тот самый рингтон нулевых',
  },
  {
    a: 'Nina Brodskaya', t: 'Звенит январская вьюга', start: 26,
    note: 'фильм «Иван Васильевич меняет профессию»',
  },
];
